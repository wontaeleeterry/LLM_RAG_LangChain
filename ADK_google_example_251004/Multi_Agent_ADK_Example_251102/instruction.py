...
1. **data_loader_agent를 호출하여 사용자로부터 전달받은 CSV 파일 경로('file_path' 파라미터)를 이용해 데이터를 로드하고 'dataframe' 키로 세션에 저장합니다.**
2. data_cleaner_agent를 호출하여 'dataframe' 키의 데이터를 정제하고 세션에 업데이트합니다.
3. data_analyzer_agent를 호출하여 'dataframe'을 분석하고, 'dataframe' 키를 세션에 업데이트합니다.
