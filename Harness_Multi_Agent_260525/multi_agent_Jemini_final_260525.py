"""
멀티 에이전트 하네스 예제 (병렬 처리 및 실시간 스트리밍 버전)
──────────────────────────────────────────
asyncio를 활용하여 3개 에이전트를 동시에 실행하고
하네스 에이전트가 결과를 실시간으로 스트리밍하며 종합합니다.
"""
...
def make_llm(streaming: bool = False) -> ChatOpenAI:
    # LangChain v0.3 이상부터는 콜백 핸들러 없이 streaming=True만으로도 
    # astream()을 통한 실시간 스트리밍이 깔끔하게 지원됩니다.
    return ChatOpenAI(
        base_url="http://localhost:1234/v1",
        api_key="lm-studio",
        model="lmstudio-community/gemma-3-12b",
        temperature=0.1,
        streaming=streaming,
    )
...
def build_agent_chain(system_prompt: str, human_template: str):
    ...
    return prompt | make_llm(streaming=False) | StrOutputParser()
...
async def run_agents_parallel(topic: str) -> tuple[str, str, str]:
    ...
    # asyncio.gather에 코루틴을 직접 전달하여 병렬 실행 명시화
    research, analysis, strategy = await asyncio.gather(
        researcher_chain.ainvoke({"topic": topic}),
        analyst_chain.ainvoke({"topic": topic}),
        strategist_chain.ainvoke({"topic": topic})
    )
...
async def run_pipeline_async(topic: str) -> str:
    ...
    # ── Step 2: 하네스 종합 (실시간 스트리밍 출력 적용) ────────────────
    print("\n[하네스 에이전트: 종합 보고서 생성 중...]\n")
    print("=" * 60)

    final_chunks = []
    # astream을 사용하여 토큰이 생성될 때마다 바로 출력합니다.
    async for chunk in harness_chain.astream({
        "topic":    topic,
        "research": research,
        "analysis": analysis,
        "strategy": strategy,
...
# 진입점
# ─────────────────────────────────────────────
TOPIC = "중소기업의 AI 도입 전략"   # 단일 LLM 실행 시 동일한 결과를 얻기위한 프롬프트 작성해보기(260525)
...
async def main():
    report = await run_pipeline_async(TOPIC)
    save_report(report)


if __name__ == "__main__":
    # 일반적인 .py 스크립트 실행 환경용 진입점 표준화
    try:
        asyncio.run(main())
    except RuntimeError:
        # Jupyter 환경 등 이미 루프가 돌고 있는 경우에 대한 셰이프 처리
        import nest_asyncio
        nest_asyncio.apply()
